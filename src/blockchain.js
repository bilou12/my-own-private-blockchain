...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        ...
        return new Promise((resolve) => {
            let msgToBeSigned = address.toString();
            msgToBeSigned += ":";
            msgToBeSigned += new Date().getTime().toString().slice(0,-3);
            msgToBeSigned += ":starRegistry";
            resolve(msgToBeSigned);
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const elapsedTimeInSeconds = timeNow - timeInMessage;
            if ( elapsedTimeInSeconds < 60 * 5) {
                ...
                reject({msg: "The message is outdated and more than 5mn"});
            }
        });
    }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            for (const b of self.chain) {
                ...
                const bData = b.getBData();
                bData.then(value => {
                    const actualAddress = value.address;
                    if (actualAddress === address) {
                        stars.push(value.star);
                    }
                });
            }
            resolve(stars);
